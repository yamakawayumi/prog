...
def create_txt():
    # diclist_data = {}
    # print(no_1_shop_name ,no_1_shop_name ,no_1_shop_url ,station_name ,station_lon ,station_lat ,train_line ,moryori_train_name ,no1_shop_name,no1_url,no1_rating_score )
    # atai = '{'+"'"+'駅名'+"'"+':'+station_name+','+"'"+'路線'+"'"+':'+train_line+','+"'"+'lat'+"'"+':'+station_lat+','+"'"+'lon'+"'"+':'+station_lon+','+"'"+'店名'+"'"+':'+no1_shop_name+','+"'"+'評価'+"'"+':'+no1_rating_score+','+"'"+'URL'+"'"+':'+no1_url+','+"'"+'最寄り'+"'"+':'+moryori_train_name+"'"+'}'
    # print(atai)
    os.makedirs('./train',exist_ok=True)
    atai = station_name+','+train_line+','+station_lat+','+station_lon+','+no1_shop_name+','+no1_rating_score+','+no1_url+','+moryori_train_name
    # koumoku = 'station_name','train_line','station_lat','station_lon','no1_shop_name','no1_rating_score','no1_url','moyori_train_name']
    # atai = [station_name,train_line,station_lat,station_lon,no1_shop_name,no1_rating_score,no1_url,moryori_train_name]
    # for i in range(len(koumoku)):
    #     diclist_data[koumoku[i]] = atai[i]#追加している
    # with open('./train/'+station_name+'.json', "w",encoding='utf-8') as f:
    #     json.dump(diclist_data, f,ensure_ascii=False)#辞書dictをjson形式にする encoding='utf8'
    # print(diclist_data)
...
def area_tabelg_hyouka_get(station_name):#駅名の周りにある評価の高いスイーツ点を取得する
    ...
    ranking_shop_url_1 = soup_html.find_all('a', class_='navi-rstlst__label navi-rstlst__label--rank') #ランキング順に整列されているページのURLを探す
    # print(ranking_shop_url_1)
    ranking_shop_url_1 = ranking_shop_url_1[0].get('href') #ランキング順に整列されているページのURLを取得する
    site = requests.post(ranking_shop_url_1)#ランキング順に整列されているページを取得
    soup_html_ranking = BeautifulSoup(site.text, 'html.parser')#この1行でHTMLからデータを引き出すことができる
    # print(ranking_shop_url_1)
    # ranking_shop_url_1 = requests.get(ranking_shop_url_1)#ランキング順に整列されている一番上のNo1人気ページのURLにアクセスする
    # print(ranki)
    #ランキングNo.1の下のタグから
    ...
    no1_shop = soup_html_ranking.find_all('a', class_='list-rst__rst-name-target cpy-rst-name js-ranking-num') #ランキング順に整列されているaタグを探す
...
#----------------------------------------------------
    ...
    for line in result:#{'name': '中野', 'prefecture': '東京都', 'line': '東京メトロ東西線', 'x': 139.665835, 'y': 35.705765, 'postal': '1640001', 'prev': None, 'next': '落合'}
        ...
        station_lon = str(line['x'])#経度
        station_lat = str(line['y'])#緯度
        station_all.append(station_name)
        area_tabelg_hyouka_get(station_name)#駅名の周りにある評価の高いスイーツ点を取得するする関数
        create_txt()
